...
function getIncomingValue(message) {
  ...
  if (message.type === "audio") {
    return {
      type: "audio",
      text: "",
      id: String(message.audio?.id || "").trim(),
    };
  }
  ...
  try {
    ...
    if (session.step === "wait_photo") {
      if (incoming.type !== "image") {
        await sendTextMessage(
          phone,
          "Я ожидаю фото зоны.\nЕсли хотите продолжить без фото, просто напишите: Привет"
        );
        return res.sendStatus(200);
      }
    ...
    if (session.step === "ask_time") {
      ...
      await sendTextMessage(
        ...
        `Спасибо, ${payload.name}! 😊\n\n` +
          ...
          `Если захотите начать заново или оформить новую заявку, просто напишите: Привет`
      );
